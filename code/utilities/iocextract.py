...
def extract_iocs_dict(data, refang=False, strip=False):
    """Extract all IOCs.
    ...
    :rtype: :py:func:`itertools.chain`
    """
    return {"url": extract_urls(data, refang=refang, strip=strip), "ip": extract_ips(data, refang=refang),
            "email": extract_emails(data, refang=refang), "md5": extract_md5_hashes(data),
            "sha1": extract_sha1_hashes(data), "sha256": extract_sha256_hashes(data),
            "sha512": extract_sha512_hashes(data)}
